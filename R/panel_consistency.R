...
#' year_range <- Scorecard %>% dplyr::group_by(unitid) %>%
...
panel_fill <- function(.df,.set_NA=FALSE,.min=NA,.max=NA,.backwards=FALSE,.group_i=TRUE,.flag=NA,.i=NA,.t=NA,.d=1,.uniqcheck=FALSE,.setpanel=TRUE) {
  ...
  newidname <- 'newidname'
  while (newidname %in% names(tocopy)) { newidname <- paste(newidname,'.',sep='')}
  ...
  tocopy <- tocopy %>%
    dplyr::mutate_at(.t,.funs=function(x)
      if (.backwards == FALSE) { tocopy[[.t]]+tocopy[[newidname]]*inp$d }
      else {tocopy[[.t]] - tocopy[[newidname]]*inp$d})
...
panel_locf <- function(.var,.df=get(".", envir=parent.frame()),.fill=NA,.backwards=FALSE,.resolve='error',.group_i=TRUE,.i=NA,.t=NA,.d=1,.uniqcheck=FALSE) {
  ...
  #We only need these
  .df <- .df %>% dplyr::ungroup() %>% dplyr::select_at(arrnames)
  ...
  if (is.character(.resolve)) {
    if (max((.df %>%
             dplyr::mutate_at(worknames[1],
                              .funs=function(x) dplyr::n_distinct(x)))[[worknames[1]]]) > 1) {
...
#' Scorecard <- fixed_force(Scorecard,.var=c('pred_degree_awarded_ipeds','state_abbr'),
